...
# Collect all arguments for the java command, stacking://them before/after Java 9 options.
...
# Stop when "xeli" is enabled so that the user can easily see/debug what happens
if ! "$cygwin" && ! "$msys" ; then
